...
function buildBaseEventQuery (params) {
    var query = [
        `MERGE (ev:Event { event_id : '${params.event_id}', type: '${params.event_type}' })`,
        `ON CREATE SET ev.original_timestamp = '${params.timestamp}'`, 
        `ON MATCH SET ev.redelivered_timestamp = '${params.timestamp}'`, 
...
function buildCommitQuery (event_id, branch, commit) {
    var query = [
        ...
        `MERGE (commit:Commit { commit_id : '${commit.id}', timestamp: '${commit.timestamp}', author_email: '${commit.author.email}' })`,
